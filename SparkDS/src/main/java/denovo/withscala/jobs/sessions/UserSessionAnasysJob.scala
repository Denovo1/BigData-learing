package denovo.withscala.jobs.sessions
...
import denovo.withjava.bean.{CategoryBean, Task, TaskParam}
import denovo.withjava.bean.session.{SessionAggrStat, SessionDetail, SessionRandomExtract, Top10Category, Top10CategorySession}
...
import denovo.withjava.dao.session.{ISessionAggrStat, ISessionDetail, ITop10Category, ITop10CategorySession}
import denovo.withjava.dao.session.impl.{SessionAggrStatImpl, SessionDetailImpl, SessionRandomExtractImpl, Top10CategoryImpl, Top10CategorySessionImpl}
import denovo.withjava.mock.MockData
import denovo.withjava.utils.{ResourcesUtils, StringUtils}
import org.apache.spark.broadcast.Broadcast
import org.apache.spark.rdd.RDD
import org.apache.spark.sql.types.{StringType, StructField, StructType}
import org.apache.spark.sql.{Row, SparkSession}
import denovo.withjava.bean.session._
import denovo.withjava.bean.TaskParam
import denovo.withjava.dao.session.ISessionAggrStat
import denovo.withjava.dao.session.impl._
import denovo.withjava.utils.StringUtils
...
object UserSessionAnasysJob {
  ...
  def randomExtract1000Session(spark: SparkSession, args: Array[String]): Unit = {
    ...
    val totalSessionCnt: Long = spark.sql("select count(*) totalSessionCnt " +
      "from filter_after_action").first.getAs[Long]("totalSessionCnt")

    val rdd: RDD[Row] = spark.sql("select  substring(action_time,1,13) timePeriod, count(*)/" +
      totalSessionCnt.toDouble + " rateValue  from filter_after_action group by substring(action_time,1,13)").rdd
  ...
  def getStepLenAndTimeLenRate(spark: SparkSession, args: Array[String]): Unit = {
    ...
    val rdd: RDD[Row] = spark.sql("select count(*) stepLen,getTimeLen(max(action_time),min(action_time)) timeLen " +
      "from filter_after_action group by session_id").rdd
    //准备一个自定义累加器的实例，并进行注册
